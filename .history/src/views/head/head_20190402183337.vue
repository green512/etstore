<template>
  <div id="header" class="new_header">
        ...
        <div class="hd_nav_bd cle">
            ...
            <div class="nav">
                <ul class="sub_nav cle" id="sub_nav">
                    <li>
                        <router-link to="/app/home/index">{{$t("login.homepage")}}</router-link>
                    </li>
                    ...
                    <li>                 
                        <router-link to="/app/doc">{{$t("login.documents")}}</router-link>
                    </li>
                </ul>
            ...
            <div class="r_bar"> 
                <div class="search_box">
                        ...
                        <button  class="sea_submit" @click="searchSubmit"><img src="./images/so-bt.png"></button>
                    </div>                   
                    <ul id="userinfo-bar">
                        ...
                        <li class="more-menu" @mouseover="isShowVip=true" @mouseout="isShowVip=false"> <a>{{$t("login.usercenter")}}</a><i class="iconfont arrow"> </i>
                            <div class="more-bd" :class="{show:isShowVip}">
                                <div class="list">
                                <a href="myproject.html" target="_blank">{{$t("login.myproject")}}</a>
                                <a href="http://127.0.0.1:8889/lab" target="_blank">{{$t("login.myworkbench")}}</a>
                                <a href="mycollection.html" target="_blank">{{$t("login.mycollection")}}</a>
                                <a href="myorder.html" target="_blank">{{$t("login.myorder")}}</a>
            ...
            <!----------<div class="hd_cart" id="ECS_CARTINFO"  v-if="userInfo.name"  @mouseover="overShopCar" @mouseout="outShopCar">
             <router-link class="tit" :to="'/app/shoppingcart/cart'" target = _blank>

                    <b class="iconfont">&#xe600;</b><span><i class="iconfont">&#xe645;</i></span>
    ...
    <div class="hd_main cle">      
        ...
        <div class="head_search_hot"><span>推荐搜索词：</span>
          <router-link v-for="item in hotSearch" :to="'/app/home/search/'+item.keywords" :key="item.keywords">
            {{item.keywords}}
          </router-link>
        </div>
        <div class="intro">
            <span>ETWatch是面向流域规划与管理和农业水管理的实用需求,针对遥感应用而设计的遥感蒸散监测系统</span>            
        </div>
    </div>
    </div>
</template>
...
.r_bar ul#userinfo-bar {
    float:right
}
.r_bar ul#userinfo-bar li .vipico {
    width:12px;
    height:17px;
...
.r_bar li.more-menu a {
    color:#fff
}
.r_bar #ECS_MEMBERZONE a{
    color: #fff
}
.r_bar li.more-menu i.arrow {
    position:absolute;
    top:5px;
    right:5px;
    font-size:16px;
...
.r_bar li.hover a.menu-link {
    color:#0e90d2
}
.r_bar li.hover i.arrow {
    -moz-transform:rotate(180deg);
    -webkit-transform:rotate(180deg);
    -ms-transform:rotate(180deg);
    transform:rotate(180deg)
}
.r_bar li.hover .more-bd {
    top:27px;
    opacity:1;
    visibility:visible
}
...
.r_bar .more-bd .hezuo_list {
    width:72px
}
.r_bar .more-bd .list a {
    display:block;
    background-color:#000;
...
.hufu-step a.no1:hover,#current-hstep1 a.no1{background-position:-52px -54px;color:#0e90d2}
.hufu-step a.no2{width:122px}
.hufu-step a.no2:hover,#current-hstep2 a.no2{background-position:-158px -54px;color:#0e90d2}
.hufu-step a.no3{width:109px}
.hufu-step a.no3:hover,#current-hstep3 a.no3{background-position:-280px -54px;color:#0e90d2}
.hufu-step a.no4{width:112px}
.hufu-step a.no4:hover,#current-hstep4 a.no4{background-position:-389px -54px;color:#0e90d2}
.hufu-step a.no5{width:117px}
.hufu-step a.no5:hover,#current-hstep5 a.no5{background-position:-501px -54px;color:#0e90d2}
.hufu-step a.no6{width:120px}
.hufu-step a.no6:hover,#current-hstep6 a.no6{background-position:-618px -54px;color:#0e90d2}
.hufu-step a.no7{width:104px}
.hufu-step a.no7:hover,#current-hstep7 a.no7{background-position:-738px -54px;color:#0e90d2}
.hufu-step a.no8{width:124px;padding-top:55px}
.hufu-step a.no8:hover,#current-hstep8 a.no8{background-position:-842px -54px;color:#0e90d2}
.caiz-step{background-position:0 -108px;background-color:#fdfdfd}
.caiz-step a.no1{width:88px}
.caiz-step a.no1:hover,#current-cstep1 a.no1{background-position:-52px -162px;color:#0e90d2}
.caiz-step a.no2{width:92px}
.caiz-step a.no2:hover,#current-cstep2 a.no2{background-position:-140px -162px;color:#0e90d2}
.caiz-step a.no3{width:84px}
.caiz-step a.no3:hover,#current-cstep3 a.no3{background-position:-232px -162px;color:#0e90d2}
.caiz-step a.no4{width:92px}
.caiz-step a.no4:hover,#current-cstep4 a.no4{background-position:-316px -162px;color:#0e90d2}
.caiz-step a.no5{width:86px}
.caiz-step a.no5:hover,#current-cstep5 a.no5{background-position:-408px -162px;color:#0e90d2}
.caiz-step a.no6{width:85px}
.caiz-step a.no6:hover,#current-cstep6 a.no6{background-position:-494px -162px;color:#0e90d2}
.caiz-step a.no7{width:95px}
.caiz-step a.no7:hover,#current-cstep7 a.no7{background-position:-579px -162px;color:#0e90d2}
.caiz-step a.no8{width:82px}
.caiz-step a.no8:hover,#current-cstep8 a.no8{background-position:-674px -162px;color:#0e90d2}
.caiz-step a.no9{width:90px}
.caiz-step a.no9:hover,#current-cstep9 a.no9{background-position:-756px -162px;color:#0e90d2}
.caiz-step a.no10{width:120px;padding-top:55px}
.caiz-step a.no10:hover,#current-cstep10 a.no10{background-position:-846px -162px;color:#0e90d2}
...
.cate-menu dd.current a,.cate-menu dd.current a:hover{color:#0e90d2;background-color:#f1f1f1}
...
.fixed-want dd a.red{color:#0e90d2;background-color:#ffecf2;border-radius:3px}
...
.search-selected a.item{border:1px solid #ddd;background-color:#fff;margin-right:8px;background-position:right -245px;box-shadow:0 1px 1px #eee;color:#0e90d2}
...
.search-options dd .items a:hover{color:#0e90d2}
...
.search-options dd a.more-btn{position:absolute;top:3px;right:5px;margin-right:0;padding:0 20px 0 0;background-position:30px -381px;display:none;color:#0e90d2}
...
.sort .curr .search_DESC{border:1px solid #fff;padding:4px 26px 4px 15px;background-position:right -104px;background-position:right -102px;color:#0e90d2;text-shadow:1px 1px 1px #fff}
.sort .curr .search_ASC{border:1px solid #fff;padding:4px 26px 4px 15px;background-position:right -330px;background-position:right -328px;color:#0e90d2;text-shadow:1px 1px 1px #fff}
...
.search_num span.search_btn a:hover{border-color:#ccc;box-shadow:0 1px 1px #eee;text-decoration:none;color:#0e90d2}
.search_num span.search_btn span{color:#333}
.search_num span.search_btn span em{color:#0e90d2}
...
.cms-box .bd a:hover span.name{color:#0e90d2}
.baokuan-goods a,.baokuan-goods a span{display:block}
.baokuan-goods a span.price{color:#0e90d2}
...
.seemore_items h3 a:hover{text-decoration:none;color:#0e90d2}
